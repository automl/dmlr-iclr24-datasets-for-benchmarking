...
class IndexLoader:
    def __init__(self, train_size: int, batch_size: int, shuffle: bool, device: torch.device) -> None:
    ...
    def __iter__(self):
        indices = list(zero.iloader(self._train_size, self._batch_size, shuffle=self._shuffle))
...
) -> ty.Tuple[ty.Optional[optim.lr_scheduler._LRScheduler], ty.Dict[str, ty.Any], ty.Optional[int],]:
...
def get_epoch_parameters(train_size: int, batch_size: ty.Union[int, str]) -> ty.Tuple[int, int]:
    if isinstance(batch_size, str):
        if batch_size == "v3":
            batch_size = 256 if train_size < 50000 else 512 if train_size < 100000 else 1024
        elif batch_size == "v1":
            batch_size = (
                16
                if train_size < 1000
            ...
            batch_size = 512 if train_size < 100000 else 1024 if train_size < 500000 else 2048
...
def _learn_with_virtual_batch(model, optimizer, loss_fn, step, batch, chunk_size) -> Tensor:
...
def get_gradient_norm_ratios(m: nn.Module):
    return {k: v.grad.norm() / v.norm() for k, v in m.named_parameters() if v.grad is not None}
...
class AdaBelief(optim.Optimizer):
    ...
    def __init__(
        ...
        if print_change_log:
            print("Please check your arguments if you have upgraded adabelief-pytorch from version 0.0.5.")
            print("Modifications to default arguments:")
            default_table = [
